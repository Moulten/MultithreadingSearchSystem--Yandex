...
void TestDuration(){
		...
		ifstream input("voyna-i-mir-tom-1.txt");
		string text;
		string line;
		for(int i = 0; i<400 && getline(input,line);i++){
			text+=(line+"\n");

		}
		stringstream new_base(text);
		string text2;
		for(int i = 0; i<400 && getline(input,line);i++){
			text2+=(line+"\n");
	...
	{
		LOG_DURATION("input ");
		SearchServer hope(new_base);
	}
	SearchServer hope(new_base);
	istringstream new_query(text2);
	{
		ostringstream output;
		LOG_DURATION("output ");

		hope.AddQueriesStream(new_query,output);
		//cout<<output.str();
	}
